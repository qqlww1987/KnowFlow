...
export function getTeamMembersApi(teamId: number) {
  return request({
    url: `api/knowflow/v1/teams/${teamId}/members`,
    method: 'get',
  });
}

// 添加团队成员
export function addTeamMemberApi(data: {
  teamId: number;
  userId: number;
  role: string;
}) {
  return request({
    url: `api/knowflow/v1/teams/${data.teamId}/members`,
    method: 'post',
    data,
  });
}
...
// 移除团队成员
export function removeTeamMemberApi(data: {
  teamId: number;
  memberId: number;
}) {
  return request({
    url: `api/knowflow/v1/teams/${data.teamId}/members/${data.memberId}`,
    method: 'delete',
  });
}
